...
"""
    ...
    def function_monitor(self, func):
        ...
        def wrapper(*args, **kwargs):
            ...
            self.bot_logger.info(f"🟢 EXECUTING: {module_name}.{function_name}")
            ...
            try:
                ...
                self.bot_logger.info(f"✅ SUCCESS: {module_name}.{function_name} completed in {execution_time:.2f}s")
                return result
                
            except Exception as e:
                self.track_error(e, f"Function {module_name}.{function_name} failed")
                raise
            ...
            with open(error_file, 'r', encoding='utf-8', errors='ignore') as file:
                ...
                for line in lines:
                    if '🚨 🚨 🚨 CRITICAL ERROR DETECTED' in line:
                        ...
                        if '📍 ERROR ORIGIN:' in line and line.strip().endswith(')'):
