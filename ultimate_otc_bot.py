...
TIMEFRAMES = ["1m", "5m", "10m", "1h", "4h"]  # Expanded timeframes
...
class UltimateAutoTradingAI:
    ...
    def generate_low_confidence_signals(self, price_action, indicators):
        """LOW CONFIDENCE STRATEGY (60%+)"""
        total_confidence = price_action['confidence_boost'] + indicators['indicator_confidence']
        signal = "HOLD"
        
        # SIMPLE RULES FOR LOW CONFIDENCE
        if ("BULLISH_ENGULFING" in price_action['price_signals'] or 
            "HAMMER_PINBAR" in price_action['price_signals']):
            if "RSI_OVERSOLD" in indicators['indicator_signals']:
                signal = "CALL"
                total_confidence += 15
                
        elif ("BEARISH_ENGULFING" in price_action['price_signals'] or 
              "SHOOTING_STAR" in price_action['price_signals']):
            if "RSI_OVERBOUGHT" in indicators['indicator_signals']:
                signal = "PUT"
                total_confidence += 15
    ...
    def generate_high_confidence_signals(self, price_action, indicators):
        """HIGH CONFIDENCE STRATEGY (80%+)"""
        total_confidence = price_action['confidence_boost'] + indicators['indicator_confidence']
        signal = "HOLD"
    ...
    def generate_auto_signal(self, price_action, indicators, current_price, pair):
        """MULTI-LEVEL SIGNAL GENERATION WITH 3 STRATEGIES"""
        
        # Run all three strategy levels
        low_signal = self.generate_low_confidence_signals(price_action, indicators)
        medium_signal = self.generate_medium_confidence_signals(price_action, indicators)
        high_signal = self.generate_high_confidence_signals(price_action, indicators)
...
⏰ **Alert Time (UTC+7):** {self.get_utc7_time().strftime('%H:%M:%S')}
    ...
    async def scan_markets(self):
        """Scan all markets for multi-level setups"""
        if not self.auto_trade_enabled:
            return
            
        logger.info("🔍 Scanning markets with MULTI-LEVEL strategies...")
        ...
        for pair in TRADING_PAIRS.keys():
            for timeframe in TIMEFRAMES:
                try:
                    ...
                    # Send alert if any confidence level triggers
...
def main():
    ...
    print("🤖 MULTI-LEVEL STRATEGY BOT ACTIVATED!")
    print("🎯 Confidence Levels: 60%+/70%+/80%+")
